...
- **작성 기준: `prompts/script.md`** — 구조·규칙·톤은 전부 그 파일이 정본.
  스타일을 바꾸고 싶으면 그 파일만 수정하면 다음 상품부터 반영된다.
...
"[id] 캡션 써줘" — 인스타 알고리즘 스킬 참조.

- **작성 기준: `prompts/caption.md`** — 구조(훅→본문→참여유도→CTA→해시태그→고지문)의 정본.
...
파트너스 링크 생성 — API 키(.env의 `PARTNERS_ACCESS_KEY/SECRET_KEY`)가 있으면:

    python scripts/partners.py "https://www.coupang.com/vp/products/..."

키가 없으면 partners.coupang.com 워크스페이스에서 수동 변환. 링크 확보 후:

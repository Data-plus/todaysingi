...
파이프라인의 상품 발굴(1번)은 **사람이 쿠팡 링크를 직접 입력하는 수동 모드가 기본**이고, 파트너스 API 자동 발굴(골드박스·베스트)은 보조 수단이다 — 신기한 물건을 고르는 감각은 사람이 낫다는 판단(사용자 결정, 2026-07-12).
...
**비범위(v2 이후):** 카테고리/필터, 상품 상세 페이지, 다크 모드(화이트 컨셉 고정), 관리자 UI, Supabase 등 백엔드, 파이프라인 자체. (검색은 v1.1로 편입 — 문서 말미 참조)
...
## v1.1 변경 (2026-07-12, 사용자 요청): 검색창

- 프로필 아래·상품 리스트 위에 검색 입력. 입력 즉시 클라이언트 필터링(서버·디바운스 없음).
- 매칭: 표시명 `[001] 제목` 기준 대소문자 무시 부분일치 — 제목과 숏츠 노출 번호 양쪽으로 검색됨.
- 결과 0개면 `#status`에 "검색 결과가 없어요." 표시, 검색어 삭제 시 전체 목록 복귀.
- 노출 조건: 활성 상품 `SEARCH_MIN_ITEMS`(=5)개 이상일 때만 표시.
- `renderProducts`는 재렌더를 위해 시작 시 `replaceChildren()`으로 목록을 비운다.

...
export function summarizeGa4(
  ...
  const daily = new Map<string, { date: string; clicks: number; sessions: number; activeUsers: number }>();
  ...
  for (const row of productRows) {
    ...
    totalClicks += clicks;
    const day = daily.get(row.metricDate) || { date: row.metricDate, clicks: 0, sessions: 0, activeUsers: 0 };
    day.clicks += clicks;
  ...
  const productSummary = Array.from(products.values())
    .map((row) => ({ ...row, share: totalClicks ? (row.clicks / totalClicks) * 100 : 0 }))
    .sort((a, b) => b.clicks - a.clicks || a.itemId.localeCompare(b.itemId));
  const sourceSummary = Array.from(sources.values())
    .map((row) => ({ ...row, share: sessions ? (row.sessions / sessions) * 100 : 0 }))

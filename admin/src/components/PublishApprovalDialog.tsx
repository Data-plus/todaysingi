  ...
  return (
    <div className="dialog-backdrop publish-approval-backdrop" onMouseDown={() => !busy && onClose()}>
      <section className="publish-approval-dialog" role="dialog" aria-modal="true" aria-labelledby="publish-approval-title" aria-describedby="publish-approval-warning" onMouseDown={(event) => event.stopPropagation()}>
        <header>
          <div><p>FINAL APPROVAL</p><h2 id="publish-approval-title">Instagram에 게시할까요?</h2><span>승인 후에는 Cloud Worker가 실제 계정에 릴스를 게시합니다.</span></div>
        ...
        <div className="publish-approval-product">
          ...
          <div className="publish-product-copy">
            ...
            <dl>
              ...
              <div><dt>Cloud Worker</dt><dd className={workerOnline ? "worker-ready" : "worker-waiting"}>{workerOnline ? "실행 중 · 곧 처리" : "요청 시 자동 실행"}</dd></div>
            </dl>
          </div>
        </div>
        ...
        <div className="publish-approval-checks" aria-label="게시 승인 확인 사항">
          ...
          <article><span><Icon name="cpu" size={16}/></span><div><strong>{workerOnline ? "Cloud Worker 실행 중" : "Cloud 자동 실행"}</strong><p>{workerOnline ? "승인 작업을 곧 가져갑니다." : "PC와 관계없이 Cloud Run이 작업을 가져갑니다."}</p></div></article>
        </div>

  ...
  return (
    <div className="page-stack">
      ...
      <section className="section-block">
        <div className="section-title-row compact"><div><p className="section-kicker">CLOUD EXECUTIONS</p><h2>Cloud Worker 실행 기록</h2><p>작업 요청 시 Cloud Run Job이 켜지고 완료 후 자동 종료됩니다.</p></div></div>
        <div className="table-card"><table className="data-table workers-table"><thead><tr><th>Worker</th><th>상태</th><th>현재 작업</th><th>버전</th><th>마지막 신호</th></tr></thead><tbody>{workers.length ? workers.map((worker) => <tr key={worker.id}><td data-label="Worker"><span className="worker-name"><Icon name="cpu" size={18}/><span><strong>{worker.name}</strong><small>{worker.id}</small></span></span></td><td data-label="상태"><StatusBadge status={worker.online ? "online" : "offline"} label={worker.online ? worker.statusLabel : "종료됨"}/></td><td data-label="현재 작업">{worker.currentJobId ? worker.currentJobId.slice(0, 8) : "—"}</td><td data-label="버전">{worker.version || "미기록"}</td><td data-label="마지막 신호">{formatDateTime(worker.lastSeenAt)}</td></tr>) : <tr><td colSpan={5}><div className="empty-state"><Icon name="cpu" size={25}/><strong>아직 Cloud 실행 기록이 없습니다.</strong><p>첫 작업을 요청하면 이곳에 실행 기록이 표시됩니다.</p></div></td></tr>}</tbody></table></div>
      </section>

  ...
  if (!rows.length) {
    return (
      <div className="performance-empty" role="status">
        <Icon name="chart" size={24}/><strong>아직 수집된 GA4 데이터가 없습니다.</strong>
    ...
    <>
      <figure className="performance-chart" aria-labelledby="traffic-chart-title traffic-chart-caption">
        <figcaption id="traffic-chart-caption" className="chart-legend">
          <span><i className="legend-clicks"/>상품 클릭</span><span><i className="legend-sessions"/>세션</span>
        </figcaption>
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="날짜별 상품 클릭과 세션 선 그래프">
          {[0, 0.5, 1].map((ratio) => <line key={ratio} className="chart-gridline" x1={paddingX} x2={width - paddingX} y1={paddingY + ratio * (height - paddingY * 2)} y2={paddingY + ratio * (height - paddingY * 2)}/>) }
          ...
          {rows.map((row, index) => labelIndexes.has(index) ? <text key={row.date} className="chart-axis-label" x={x(index)} y={height - 3} textAnchor="middle">{shortDate(row.date)}</text> : null)}
        </svg>
      </figure>
      <details className="chart-data-table">
        <summary>차트 수치 표로 보기</summary>
        <div className="table-card"><table className="data-table"><thead><tr><th>날짜</th><th>상품 클릭</th><th>세션</th><th>활성 사용자 합계</th></tr></thead><tbody>{rows.map((row) => <tr key={row.date}><td>{row.date}</td><td>{formatNumber(row.clicks)}</td><td>{formatNumber(row.sessions)}</td><td>{formatNumber(row.activeUsers)}</td></tr>)}</tbody></table></div>
      </details>
    </>
  );
}
    ...
    <div className="page-stack performance-page">
      <section className="performance-hero">
        <div>
          ...
          <div className="performance-sync-meta"><span className={`sync-dot sync-${ga4?.status || "waiting"}`}/><strong>{ga4?.label || "연결 대기"}</strong><span>마지막 성공 {lastSync}</span>{sync?.rangeStart && sync.rangeEnd ? <span>{sync.rangeStart}–{sync.rangeEnd}</span> : null}</div>
        </div>
        <div className="performance-hero-actions">
          <button type="button" className="primary-button" disabled={!live || syncing || syncRunning} onClick={onSync}><Icon name="refresh" size={17}/>{syncing || syncRunning ? "동기화 중…" : "GA4 새로고침"}</button>
          <button type="button" className="secondary-button" onClick={onOpenSettings}><Icon name="settings" size={17}/>연동 설정</button>
        </div>
      </section>
      ...
      <section className="section-block">
        <div className="section-title-row compact"><div><p className="section-kicker">DAILY TREND</p><h2 id="traffic-chart-title">클릭과 방문 추이</h2><p>상품 클릭과 사이트 세션을 같은 날짜 축에서 비교합니다.</p></div><span className="data-period"><Icon name="calendar" size={15}/>최근 삼십 일</span></div>
      ...
      <section className="performance-split-grid">
        <article className="section-block performance-table-section">
          ...
          <div className="table-card"><table className="data-table performance-rank-table"><thead><tr><th>상품</th><th>클릭</th><th>비중</th></tr></thead><tbody>
            {productRows.length ? productRows.map((row) => {
              const product = row.productId === null ? undefined : productById.get(row.productId);
              return <tr key={row.itemId}><td data-label="상품"><span className="rank-product"><b>{row.itemId.padStart(3, "0")}</b><strong>{product?.title || row.itemName || "미등록 상품"}</strong></span></td><td data-label="클릭"><strong>{formatNumber(row.clicks)}</strong></td><td data-label="비중"><span className="share-cell"><i style={{ width: `${Math.min(100, row.share)}%` }}/><em>{row.share.toLocaleString("ko-KR", { maximumFractionDigits: 1 })}%</em></span></td></tr>;
            }) : <tr><td colSpan={3}><div className="empty-state compact-empty"><strong>상품 클릭 데이터가 없습니다.</strong></div></td></tr>}
          </tbody></table></div>
        </article>
        ...
        <article className="section-block performance-table-section">
          ...
          <div className="table-card"><table className="data-table source-performance-table"><thead><tr><th>경로</th><th>세션</th><th>비중</th></tr></thead><tbody>
            {performance.sources.length ? performance.sources.slice(0, 10).map((row) => <tr key={`${row.source}/${row.medium}`}><td data-label="경로"><span className="source-name"><strong>{row.source}</strong><small>{row.medium}</small></span></td><td data-label="세션"><strong>{formatNumber(row.sessions)}</strong></td><td data-label="비중">{row.share.toLocaleString("ko-KR", { maximumFractionDigits: 1 })}%</td></tr>) : <tr><td colSpan={3}><div className="empty-state compact-empty"><strong>유입 경로 데이터가 없습니다.</strong></div></td></tr>}
          </tbody></table></div>
        </article>
      </section>

...
def synthesize(text, voice, rate, out_mp3):
    """TTS 엔진 경계. 오디오를 out_mp3에 저장하고 단어 타이밍 목록을 반환한다.

    반환: [{"text": str, "start_s": float, "end_s": float}, ...]
    v1: edge-tts. Typecast 전환 시 이 함수만 교체.
    """
    import edge_tts
...
def main(argv=None):
    ...
    print(f"TTS 생성 중 ({args.voice}, rate {args.rate})...")
    words = synthesize(text, args.voice, args.rate, voice_mp3)
    ...
    pipeline.advance_item(data, args.id, to="audio_ready",
                          sets={"finalVideo": rel, "voice": args.voice})
